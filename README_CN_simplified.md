...
| `action_space` | dict | 否 | 动作空间配置：`{type: "continuous" | "discrete", low, high, shape, n}` |
...
| 特征名 | 描述 |
|-------|------|
| `upstream_depth` | 上游节点水深 |
| `downstream_depth` | 下游节点水深 |
| `flow` | 链接流量 |
| `setting` | 当前控制开度 |
| `rainfall` | 配置雨量计的降雨强度 |
| `upstream_flooding` | 上游节点洪涝速率 |
| `downstream_flooding` | 下游节点洪涝速率 |
| `total_flooding` | 系统总洪涝 |
| `upstream_head` | 上游水位标高 |
| `downstream_head` | 下游水位标高 |
| `upstream_volume` | 上游存储体积 |
| `downstream_volume` | 下游存储体积 |
...
- **动作空间**：可配置——连续 `Box(0, 1)`（默认）或 `Discrete(n)`。详见[动作空间配置](#动作空间配置)。

所有智能体共享 **全局奖励**（耦合排水系统的物理特性）。

## 动作空间配置

动作空间类型由可选的 `action_space` 配置段控制。若省略，环境默认为连续 `Box(0, 1, shape=(1,))`。
...
每个智能体输出 `[0, 1]` 范围内的浮点值，直接作为 SWMM 元件的目标开度。
...
每个智能体输出整数索引 `0, 1, ..., n-1`，映射为等间距的连续值：
...
底层的 SWMM 模拟始终接收连续 `[0, 1]` 开度值——离散只是映射到同一连续控制空间的不同输入接口。
...
| `action_space` | `(agent) -> Box / Discrete` | 获取智能体的动作空间（RLlib 兼容）；取决于 `action_space` 配置 |
...
| `action_spaces` | dict | 每个智能体的动作空间（Box 或 Discrete）
...
│   ├── observation/             # 观测模块
│   │   ├── feature_extractors.py # 特征提取器注册表
│   │   ├── default_observation.py # 观测函数构建器
│   │   └── custom_observation.py # 自定义观测模板
│   ├── reward/                  # 奖励模块
