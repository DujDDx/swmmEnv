...
每個元件皆支援連續控制 `[0, 1]`（0 = 全關、1 = 全開）以及離散控制（透過 `action_space` 設定）。
...
| 特徵名 | 描述 |
|-------|------|
| `upstream_depth` | 上游節點水深 |
| `downstream_depth` | 下游節點水深 |
| `flow` | 連結流量 |
| `setting` | 目前控制開度 |
| `rainfall` | 設定雨量計的降雨強度 |
| `upstream_flooding` | 上游節點洪災速率 |
| `downstream_flooding` | 下游節點洪災速率 |
| `total_flooding` | 系統總洪災 |
| `upstream_head` | 上游水位標高 |
| `downstream_head` | 下游水位標高 |
| `upstream_volume` | 上游儲存體積 |
| `downstream_volume` | 下游儲存體積 |
| `upstream_inflow` | 上游入流速率 |
| `downstream_inflow` | 下游入流速率 |
| `link_depth` | 連結水深 |
| `link_volume` | 連結水體積 |
...
| 智能體類型 | 觀測維度 | 特徵組成 |
|-----------|---------|---------|
| pump（抽水站）| 5 | 上游水深、下游水深、流量、目前開度、降雨強度 |
| gate（閘門）| 4 | 上游水深、下游水深、目前開度、降雨強度 |
| weir（堰壩）| 4 | 上游水深、下游水深、目前開度、降雨強度 |

- **觀測空間**：`Box(-∞, +∞)`，經 z-score 正規化後
- **動作空間**：可設定——連續 `Box(0, 1)`（預設）或 `Discrete(n)`。詳見[動作空間設定](#動作空間設定)。

所有智能體共享 **全域獎勵**（耦合排水系統的物理特性）。

## 動作空間設定

動作空間類型由可選的 `action_space` 設定段控制。若省略，環境預設為連續 `Box(0, 1, shape=(1,))`。
...
每個智能體輸出 `[0, 1]` 範圍內的浮點數，直接作為 SWMM 元件的目標開度。
...
每個智能體輸出整數索引 `0, 1, ..., n-1`，對應等間距的連續值：

```
索引 0 → target_setting 0.0
索引 1 → target_setting 0.1
...
索引 n-1 → target_setting 1.0
```

底層 SWMM 模擬始終接收連續 `[0, 1]` 開度值——離散只是對應到同一連續控制空間的不同輸入介面。
...
│   ├── observation/             # 觀測模組
│   │   ├── feature_extractors.py # 特徵提取器註冊表
│   │   ├── default_observation.py # 觀測函數建構器
│   │   └── custom_observation.py # 自訂觀測模板
│   ├── reward/                  # 獎勵模組
